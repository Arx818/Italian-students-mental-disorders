...
######################TEST FOR CHANGES IN BDI SCORE#######################àà
...
#   summarise(
#     count = n(),
#     median = median(EHQtotal, na.rm = TRUE),
#     quantile1st = quantile(EHQtotal, probs = 0.25, na.rm = TRUE),
#     quantile3rd = quantile(EHQtotal, probs= 0.75, na.rm = TRUE)
#   )  

group_by(ECOS_dupl2, sex) %>%
  summarise(
    count = n(),
    median = median(EHQtotal, na.rm = TRUE),
    quantile1st = quantile(EHQtotal, probs = 0.25, na.rm = TRUE),
    quantile3rd = quantile(EHQtotal, probs= 0.75, na.rm = TRUE),
    mean = mean(EHQtotal, na.rm = TRUE),
    sd = sd(EHQtotal, na.rm = TRUE),
  )  

group_by(ECOS_dupl2, sex, anyED) %>%
  summarise(
    count = n(),
    median = median(EHQtotal, na.rm = TRUE),
    quantile1st = quantile(EHQtotal, probs = 0.25, na.rm = TRUE),
...
group_by(ECOS_dupl2, sex, anyED) %>%
  summarise(
    count = n(),
    median = median(EHQemotions, na.rm = TRUE),
    quantile1st = quantile(EHQemotions, probs = 0.25, na.rm = TRUE),
    quantile3rd = quantile(EHQemotions, probs= 0.75, na.rm = TRUE),
    mean = mean(EHQemotions, na.rm = TRUE),
    sd = sd(EHQemotions, na.rm = TRUE),
  ) 

group_by(ECOS_dupl2, sex, anyED) %>%
  summarise(
    count = n(),
    median = median(EHQconvictions, na.rm = TRUE),
    quantile1st = quantile(EHQconvictions, probs = 0.25, na.rm = TRUE),
    quantile3rd = quantile(EHQconvictions, probs= 0.75, na.rm = TRUE),
    mean = mean(EHQconvictions, na.rm = TRUE),
    sd = sd(EHQconvictions, na.rm = TRUE),
  ) 

group_by(ECOS_dupl2, sex, anyED) %>%
  summarise(
    count = n(),
    median = median(EHQproblems, na.rm = TRUE),
    quantile1st = quantile(EHQproblems, probs = 0.25, na.rm = TRUE),
    quantile3rd = quantile(EHQproblems, probs= 0.75, na.rm = TRUE),
    mean = mean(EHQproblems, na.rm = TRUE),
    sd = sd(EHQproblems, na.rm = TRUE),
  ) 
...
# dopo aver tenuto in considerazione tutti gli altri fattori (anyED, anyDisease) il sesso non è un fattore
# importante
...
#anoressia nervosa è l'unico disturbo che altera il EHQTOT
...
#ocitot si ma quanto è rilevante? Categorie di oci?
